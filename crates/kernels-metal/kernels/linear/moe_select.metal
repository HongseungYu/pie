...
[[kernel]] void select_gemv(
    ...
    // The seat each expert's weights sit in; the identity when `seated` is 0.
    const device int* seats            [[buffer(9)]],
    const constant uint& seated        [[buffer(10)]],
  ...
  const int id = routes[route];
  if (id < 0) {
    if (lane == 0) y[at] = bfloat(0);
    return;
  }
  const int e = seated ? seats[id] : id;

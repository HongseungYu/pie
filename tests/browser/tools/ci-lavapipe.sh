...
echo "== $what"
PIE_BENCH_FIXTURE=1 node scripts/bench/browser/native.mjs "ws://127.0.0.1:$port/v1/ws" "$what" | grep -v "^RESULT" | cut -c1-160
echo "== sort-probe"
PIE_MATRIX_ENTRY_TIMEOUT_MS=900000 node tests/browser/tools/matrix-native.mjs "ws://127.0.0.1:$port/v1/ws" sort-probe-light > "$work/sort-probe.log" || true
